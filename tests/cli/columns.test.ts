...
// `docx columns` is the intuitive verb for column layout: wrap a paragraph range
...
describe("docx columns — range wrap (--at pN-pM)", () => {
	test("wraps a paragraph range in its own N-column continuous section", async () => {
		...
		expect(
			(await runCli("columns", doc, "--at", "p2-p4", "--count", "2")).exitCode,
		).toBe(0);
	...
	test("a single paragraph (--at pN) is a one-paragraph range", async () => {
		...
		expect(
			(await runCli("columns", doc, "--at", "p2", "--count", "2")).exitCode,
		).toBe(0);
	...
	test("a range at the document start omits the leading break", async () => {
		...
		await runCli("columns", doc, "--at", "p0-p2", "--count", "2");
		const secs = await sections(doc);
	...
	test("rejects a range that already contains a section break", async () => {
		...
		await runCli("columns", doc, "--at", "p2-p3", "--count", "2");
...
describe("docx columns — section edit (--at sN)", () => {
	test("recounts an existing section's columns in place", async () => {
		...
		expect(
			(await runCli("columns", doc, "--at", id, "--count", "3")).exitCode,
		).toBe(0);
...
describe("docx columns — validation", () => {
	...
	test("rejects a non-integer --count instead of truncating", async () => {
		const doc = await makeDoc("columns-floatcount", 3);
		for (const bad of ["2.5", "1e2", "0x4", "abc"]) {
			const result = await runCli("columns", doc, "--at", "p1", "--count", bad);
			expect(result.exitCode).toBe(2);
			expect((result.parsed as { code: string }).code).toBe("USAGE");
		}
	...
	test("rejects a missing --at", async () => {
		...
		expect((await runCli("columns", doc, "--count", "2")).exitCode).toBe(2);
	});
});
...
describe("docx columns — body-level guard", () => {
