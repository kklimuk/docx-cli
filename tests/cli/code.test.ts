...
describe("docx code add", () => {
	...
	test("--code-file normalizes CRLF (no stray \\r in runs)", async () => {
	...
	test("--code threads spacing/indent onto every code paragraph", async () => {
		const path = join(tempWorkspace("code-add-spacing"), "doc.docx");
...
describe("docx code edit", () => {
	test("--at pN replaces a paragraph with a code block", async () => {
		const docPath = join(tempWorkspace("code-edit-single"), "out.docx");
		await runCli("create", docPath, "--text", "Intro.");
		await runCli("insert", docPath, "--after", "p0", "--text", "placeholder");
		// p1 is "placeholder"; replace it with a code block.
		await runCli(
			"code",
			"edit",
			docPath,
		...
		const blocks = await blocksOf(docPath);
		const codeBlocks = blocks.filter((b) => b.style?.startsWith("CodeBlock"));
		// Multi-line --code splits into one CodeBlock paragraph per source line.
		expect(codeBlocks).toHaveLength(2);
		const texts = codeBlocks.map(
			(b) => b.runs?.map((r) => r.text ?? "").join("") ?? "",
		);
		expect(texts).toEqual(["x = 1", "y = 2"]);
		// The original "placeholder" paragraph is gone (replaced, not appended).
	...
	test("--at pN --language re-highlights (swaps the per-language style)", async () => {
		const docPath = join(tempWorkspace("code-edit-relang"), "out.docx");
		await runCli("create", docPath, "--text", "Intro.");
		// Author a python block, then re-edit it as typescript.
		await runCli(
			"code",
			"add",
			docPath,
		...
		const codeBlock = blocks.find((b) => b.style?.startsWith("CodeBlock"));
		expect(codeBlock?.style).toBe("CodeBlock-typescript");
		// typescript keyword tokens pick up the keyword color.
		const keywordRun = codeBlock?.runs?.find((r) => r.text === "const");
		expect(keywordRun?.color).toBe("CF222E");
	});

	test("--at pN-pM replaces a range of paragraphs with a code block", async () => {
		const docPath = join(tempWorkspace("code-edit-range"), "out.docx");
		await runCli("create", docPath, "--text", "Intro.");
		await runCli("insert", docPath, "--after", "p0", "--text", "A");
		await runCli("insert", docPath, "--after", "p1", "--text", "B");
		// p1-p2 = ["A", "B"]; replace the whole range with one code paragraph.
		await runCli("code", "edit", docPath, "--at", "p1-p2", "--code", "z = 3");
		const blocks = await blocksOf(docPath);
		const codeBlocks = blocks.filter((b) => b.style?.startsWith("CodeBlock"));
		expect(codeBlocks).toHaveLength(1);
		expect(codeBlocks[0]?.runs?.map((r) => r.text ?? "").join("")).toBe(
			"z = 3",
		);
		const allText = blocks
			.flatMap((b) => b.runs ?? [])
	...
	test("--code and --code-file are mutually exclusive", async () => {
		const docPath = join(tempWorkspace("code-edit-conflict"), "out.docx");
		await runCli("create", docPath, "--text", "Intro.");
		const result = await runCli(
			"code",
			"edit",
	...
	test("missing --at is a USAGE error", async () => {
		const docPath = join(tempWorkspace("code-edit-no-at"), "out.docx");
		await runCli("create", docPath, "--text", "Intro.");
		const result = await runCli("code", "edit", docPath, "--code", "foo");
		expect(result.exitCode).toBe(2);
		expect((result.parsed as { code: string }).code).toBe("USAGE");
	});

	test("--track records the replacement as a tracked change", async () => {
		const docPath = join(tempWorkspace("code-edit-track"), "out.docx");
		await runCli("create", docPath, "--text", "Intro.");
		await runCli("insert", docPath, "--after", "p0", "--text", "placeholder");
		await runCli(
			"code",
