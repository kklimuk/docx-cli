...
All six are baked into [canonical-parts.ts](canonical-parts.ts) (the theme is a static file under `canonical/theme1.xml`, the rest inline string constants). The create flow writes them via the `CANONICAL_PARTS` loop; hand-rolled fixture scripts get them via `addCanonicalParts` in `scripts/fixture-helpers.ts`. Both iterate `Object.values(CANONICAL_PARTS)` for content-type overrides and relationships, so adding a part is usually a single edit to that map (set the right `scope`; `buildContentTypes`/`buildRootRels`/`buildDocumentRels` pick it up). New JSX tag names in a part body may need additions to the `W_TAGS` / namespace lists in `src/core/jsx/index.ts`.
