...
export async function runApply(
	...
	try {
		...
		// After a SUBSET accept/reject, the tcN/revN ids of everything left over
		// renumber — the trap that wrecks a finalize done as a sequence of single
		// calls (accept tc0, accept tc1, … then reject tc5 → "not found"). Rather
		// than hint at it, SHOW the current state: re-read the saved file and print
		// what remains with its fresh handles, so the next call addresses live ids.
		// Skipped for --all (nothing remains), --dry-run (no write), and --verbose
		// (the JSON ack is the machine contract; keep it clean). Only write when
		// there's actually something left — `writeStdout("")` would DISCARD the
		// ack just printed (Bun's stdout sink drops a prior write on an empty one).
		if (!all && !parsed.values.verbose) {
