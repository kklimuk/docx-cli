...
- [document/](document/) — `Document` ([document/index.ts](document/index.ts)) + the embedded views. Each view owns one OPC part's `XmlNode` tree plus any maps keyed to it:
  - `Body` ([document/body.ts](document/body.ts)) — `word/document.xml` body: `blocks`, `blockReferences` (pN/tN/sN, cell-chained `tN:rRcC:pK`, text-box-chained `tbxN:pK`), `imageById`, `hyperlinkById`, `equationReferences`, `textBoxReferences`. The `*iterateBlocks` walker is canonical (descends into table cells AND text-box stories; `{ textBoxes: false }` for the flow only); use it over hand-rolling.
...
**Block-level `<w:sdt>` (Word content controls) are TRANSPARENT to the block walk.** `collectBlocks` descends into `<w:sdtContent>` and reads the `<w:p>`/`<w:tbl>` inside as ordinary blocks — same shared `pN`/`tN`/`sN` counters (so ids stay sequential in document order across the boundary), and each `BlockReference.parent` is the sdtContent's own child array, so an `edit`/`insert`/`delete` splices INSIDE the control. Without this an entire control — a clause, a form field, a data-bound region — is absent from the AST: `read` can't show it, `wc` can't count it, and a `read → create` rebuild silently drops it, while the bytes sit happily on disk. (Our raw gate already refuses to WRITE a block-level `<w:sdt>` for exactly that reason; inbound Word files carry them regardless.) Each such block carries `contentControl` (`alias`/`tag`) so `read` can still say "you're editing inside a control" — Word may LOCK one against edit or deletion, so a change that succeeds here can be refused in the UI. **RUN-level `<w:sdt>` stays skipped** by `walkRunContainer`: descending there would surface nested `<w:ins>`/`<w:del>` as run-level `tcN` ids with no sensible accept/reject scope, and the checkbox shape has its own recognizer ([core/task-list](../task-list/CLAUDE.md)).

**`<mc:AlternateContent>` is transparent at every level too** (see [core/CLAUDE.md](../CLAUDE.md#markup-compatibility-mcalternatecontent-and-text-boxes)): block-level wrappers walk their chosen branch with the branch's child list as `parent` (like `<w:sdtContent>`); paragraph-level ones re-enter `walkRunContainer` with the branch as the container; run-level ones (Word's shape/text-box shape) are flattened inside `readRun`. **Text boxes** — any `<w:txbxContent>` under a `<w:drawing>`/`<w:pict>` — become a `TextBoxRun` on the anchor paragraph (`readTextBoxes`): each story is walked by `collectScopedBlocks` under a fresh `tbxN` prefix (global document order, `WalkState.textBoxIndex`), its paragraphs land in `blockReferences` with the `<w:txbxContent>` child list as parent, and `Body.textBoxReferences` records the story node + `anchorBlockId` (`WalkState.currentBlockId`, saved/restored around the nested walk). Tracked changes inside a story register `tcN` ids in the same single walk.
