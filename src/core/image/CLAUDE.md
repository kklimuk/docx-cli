...
Three files reachable via the `@core/image` barrel ([index.ts](index.ts)): [formats.ts](formats.ts) (mime↔extension table), [source.ts](source.ts) (resolve an input reference to bytes), [drawing.tsx](drawing.tsx) (the `Images` lens + the `<Image>` emitter). A fourth file, [svg-sanitize.ts](svg-sanitize.ts), is internal to `source.ts` and not surfaced through the barrel.

## Emit + package ops

`loadImageSource(src)` in [source.ts](source.ts) resolves a path / `data:` URI / `http(s)` URL to `{bytes, extension, mimeType, pixelWidth?, pixelHeight?}` (header-parsed dims for png/jpeg/gif; other formats need explicit sizing). `computeExtentEmu` turns pixels + optional inch overrides into the `<wp:extent>` EMU pair. The `Images` lens in [drawing.tsx](drawing.tsx) carries `new Images(document).list()` (finds drawing runs in `imgN` order — shared by `images replace`/`delete`) and `new Images(document).add(source)` (writes `word/media/imageN.ext`, mints the `image` relationship, registers the content-type `<Default>`). `<Image>` is the pure emitter for the inline `<w:drawing>` run.
