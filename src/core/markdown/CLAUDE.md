...
- **Image dimensions overrides** — there's no `--image-width` style flag on `--markdown`; we use the source's intrinsic pixel dimensions. Pass `--width` via standalone `docx images add` for explicit sizing.
