...
**Three render invariants, all empirically earned against Word-for-Mac (a corrupt fixture + a user's Word open with unsaved work), all load-bearing — don't regress them:**

1. **Bind the staged document BY FILENAME, never `active document`.** When a render shares an already-open Word instance and the staged open FAILS (corrupt file), `active document` falls through to the USER's document — `save as` would export their doc as our "render" and `close saving no` would close their work. `document "<staged-name>"` can only ever resolve to our staged copy; a failed open makes the lookup throw → honest `RENDER_FAILED`.
2. **`set display alerts to none` + split `with timeout` (60s open / 90s save).** A corrupt .docx pops a modal "unreadable content — recover?" that blocks the AppleEvent; alerts-off auto-declines it (a human clicking "Yes" would REPAIR the file → contaminated render). The open leash bounds the wedge; the value trades corrupt-file speed against false-failing a legitimately large document (60s open is the compromise). The Windows COM engine mirrors this with `$word.DisplayAlerts = 0` + a PowerShell-process kill timer.
3. **Clean up ONLY via `kill(2)` on render-spawned PIDs — never `killall`, never AppleScript `quit`.** `reapRenderWord` snapshots Word PIDs before the render and SIGKILLs only new ones afterward, so a Word the user already had open is never touched. `wordPids()` distinguishes pgrep exit 1 (no match) from exit ≥2 (error): on error it returns null and the reap does NOTHING (fail closed — never risk the user's instance on an ambiguous snapshot). A dedicated second instance can't be driven: with two same-bundle-id instances, Apple Event routing is nondeterministic (verified — `tell`, JXA `Application(pid)`, and frontmost-first all hit the wrong instance), which is why renders stay serialized on the single addressable instance and `kill(2)` is the only cleanup we allow.
