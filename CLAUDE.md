...
src/
  ...
  core/
    ...
    styles.tsx                # ensureStyle(view, BaselineStyleId) — lazy styles.xml provisioning + baseline catalog (Heading1-6, Quote, Code, ListParagraph, FootnoteReference/Text, etc.)
    numbering.tsx             # allocateNum(view, "bullet" | "ordered") — lazy numbering.xml provisioning, seeds bullet/ordered abstractNum with markdown-reader-tight indents
    relationships.ts          # addHyperlinkRelationship — _rels helpers (mintRelationshipId/nextRelationshipId lives in package/parts.ts)
    locators/
...
scripts/
  ...
  make-styles-injection-fixture.ts      # rebuild styles-injection.docx (body references missing styles; exercises ensureStyle)
  make-lists-fixture.ts                 # rebuild lists.docx (bulleted + ordered + 3 levels deep; dogfoods allocateNum + ListParagraph)
  fixture-helpers.ts                    # shared helpers for hand-rolled fixture scripts: wrapDocument, buildCoreProps, buildContentTypes, buildRootRels, buildDocumentRels, addCanonicalParts
...
- **`docx create` must ship Word-canonical parts**. Microsoft Word treats `word/styles.xml`, `word/settings.xml`, `word/fontTable.xml`, `word/webSettings.xml`, `word/theme/theme1.xml`, and `docProps/app.xml` as required even though ECMA-376 marks them optional. Without them Word shows an "unreadable content / recover?" prompt on open. LibreOffice is more permissive — our integration suite passed for years while Word users hit the warning. All six parts are baked into [src/cli/create/canonical-parts.ts](src/cli/create/canonical-parts.ts) (the theme is a static file under `canonical/theme1.xml`, the rest are inline string constants). The create flow writes them via the `CANONICAL_PARTS` loop; hand-rolled fixture scripts get them via `addCanonicalParts` in [scripts/fixture-helpers.ts](scripts/fixture-helpers.ts). Both consumers iterate `Object.values(CANONICAL_PARTS)` for content-type overrides and relationships, so adding a new part is usually a single edit to that map (set the right `scope` and the `buildContentTypes` / `buildRootRels` / `buildDocumentRels` helpers pick it up). New JSX tag names in the part body may need additions to the `W_TAGS` / namespace lists in [src/core/jsx/index.ts](src/core/jsx/index.ts).
- **Hand-rolled XML must avoid inter-element whitespace inside `<w:body>` and `<cp:coreProperties>`**. These parents have `complexContent` and don't allow text — Word's reader treats whitespace between child elements as illegal character data and triggers the same "unreadable content" prompt. fast-xml-builder omits inter-element whitespace by default so the JSX path is safe; only hand-rolled XML in scripts/ needs care. The `wrapDocument` / `buildCoreProps` helpers in [scripts/fixture-helpers.ts](scripts/fixture-helpers.ts) collapse via `/\n\s*/g` and emit single-line output. Pretty-print indentation in fixture sources is fine as long as it flows through these helpers.
- **Integration tests must isolate the LibreOffice user profile**. macOS `soffice --headless` locks the default user profile (`~/Library/Application Support/LibreOffice/4`); any concurrent or stale soffice causes new spawns to exit non-zero. The roundtrip test in [tests/integration/libreoffice-roundtrip.test.ts](tests/integration/libreoffice-roundtrip.test.ts) creates one `mkdtemp` profile in `beforeAll` and passes `-env:UserInstallation=file://<dir>` on every spawn. Any new test that calls `soffice` directly must follow the same pattern, or it'll show up as a flake the moment two test runs (or two test files) overlap.
...
- New baseline style? Extend the `BaselineStyleId` union in [src/core/styles.tsx](src/core/styles.tsx) and add the corresponding JSX-built definition to the `BASELINE` record. Match Word's typical attributes if possible — pStyle children must be in ECMA-376 §17.3.1.26 schema order (pStyle → keepNext → keepLines → numPr → pBdr → spacing → ind → jc). If the style references new tags (e.g. `tabs`, `shd`), add them to `W_TAGS` in [src/core/jsx/index.ts](src/core/jsx/index.ts).
- New list/number format? `allocateNum` in [src/core/numbering.tsx](src/core/numbering.tsx) currently handles `"bullet" | "ordered"` (decimal). For a new kind (e.g. roman, lettered as level-0), extend `AbstractNumKind` and add a matching abstractNum builder. The `ensureAbstractNum` detection logic keys on `lvl[0]/numFmt/w:val` — make sure each new kind has a distinct level-0 numFmt so reuse works. Update `tests/core/numbering.test.ts` to cover the new kind.
- New fixture? Drop in `tests/fixtures/`, add to the FIXTURES list in `tests/integration/libreoffice-roundtrip.test.ts` if it should round-trip. Prefer building it programmatically via a `scripts/make-*-fixture.ts` script (so the source is reproducible and stays MIT-compatible). Hand-rolled scripts MUST use the helpers in [scripts/fixture-helpers.ts](scripts/fixture-helpers.ts) (`wrapDocument`, `buildCoreProps`, `buildContentTypes`, `buildRootRels`, `buildDocumentRels`, `addCanonicalParts`) so the fixture ships the Word-canonical part set and avoids inter-element whitespace in `<w:body>` / `<cp:coreProperties>` — both of which trigger Word's "unreadable content" prompt. Scripts that dogfood the CLI (`bun ${cliEntry} create ...`) inherit canonical parts automatically. The `scripts/inspect-fixtures.ts` summarizes what each one exercises.
